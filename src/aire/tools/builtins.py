...
- ``http_get`` performs outbound network access and is therefore classified
  ``external_side_effect`` so policies can gate it.
"""
...
def _list_files(directory: str = ".", pattern: str = "**/*") -> list[str]:
    """List files under a directory (relative paths)."""
    root = Path(directory).resolve()
    if not root.is_dir():
        raise ToolError(f"not a directory: {root}")
    return sorted(str(p.relative_to(root)) for p in root.glob(pattern) if p.is_file())[:500]
